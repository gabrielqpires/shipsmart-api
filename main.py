...
AZUL      = "0D1B5E"; AZUL2     = "1A2E7C"; AMARELO   = "F5C842"
BRANCO    = "FFFFFF"; CINZA_LT  = "F0F2F8"; CINZA_MED = "D8DCE8"
VERDE_LT  = "E2EFDA"; VERDE_TX  = "375623"; VERM_LT   = "FDECEA"
VERM_TX   = "9C0006"; AMARELO_LT= "FFF8DC"; LARANJA_LT= "FCE4D6"
...
def gerar_xlsx(df_p: pd.DataFrame, df_r: pd.DataFrame) -> bytes:
    # ── Converter valores ────────────────────────────────────────────────────
    ...
    for c in val_p: df_p[c] = df_p[c].apply(pbr)
    for c in val_r: df_r[c] = df_r[c].apply(pbr)
    df_p["Previsão de Pagamento"]   = pd.to_datetime(df_p["Previsão de Pagamento"],   errors="coerce")
    df_r["Previsão de Recebimento"] = pd.to_datetime(df_r["Previsão de Recebimento"], errors="coerce")
    df_r["Último Recebimento"]      = pd.to_datetime(df_r["Último Recebimento"],      errors="coerce")
    ...
    # ── Receber ──────────────────────────────────────────────────────────────
    r_atras  = df_r[(df_r["Previsão de Recebimento"] < HOJE) & (df_r["Valor a Receber"] > 0)]["Valor a Receber"].sum()
    r_fut    = df_r[(df_r["Previsão de Recebimento"] >= HOJE) & (df_r["Valor a Receber"] > 0)]
    r_diario = r_fut.groupby("Previsão de Recebimento")["Valor a Receber"].sum()

    larr      = df_r[df_r["Cliente (Nome Fantasia)"].str.upper().str.contains("LARROUDE", na=False)]
    larr_fut  = larr[(larr["Previsão de Recebimento"] >= HOJE) & (larr["Valor a Receber"] > 0)]
    larr_diario = larr_fut.groupby("Previsão de Recebimento")["Valor a Receber"].sum()

    MES_ATRAS   = HOJE - pd.Timedelta(days=30)
    df_rec      = df_r[df_r["Valor Recebido"] > 0].copy()
    df_no_prazo = df_rec[
        df_rec["Último Recebimento"].notna() &
        df_rec["Previsão de Recebimento"].notna() &
        (df_rec["Último Recebimento"].dt.date == df_rec["Previsão de Recebimento"].dt.date) &
        (df_rec["Último Recebimento"] >= MES_ATRAS) &
        (df_rec["Último Recebimento"] <= HOJE)
    ]
    media_diaria = df_no_prazo["Valor Recebido"].sum() / 30

    # ── Pagar ────────────────────────────────────────────────────────────────
    p_atras  = df_p[(df_p["Previsão de Pagamento"] < HOJE) & (df_p["Valor a Pagar"] > 0)]["Valor a Pagar"].sum()
    p_fut    = df_p[(df_p["Previsão de Pagamento"] >= HOJE) & (df_p["Valor a Pagar"] > 0)]
    p_diario = p_fut.groupby("Previsão de Pagamento")["Valor a Pagar"].sum()

    # ── Workbook ─────────────────────────────────────────────────────────────
    wb = Workbook(); wb.remove(wb.active)

    COL_ATRAS = 4; COL_D0 = 5; LAST_COL = COL_D0 + N - 1
    ...
    for i, d in enumerate(datas):
        col = COL_D0 + i
        c = ws.cell(row=col, column=col) if False else ws.cell(row=10, column=col)
        c.value = d.to_pydatetime().date(); c.number_format = DATE_FMT
    ...
    for row, info in LINHAS.items():
        if info is None:
            ...
            for col in range(1, LAST_COL + 3):
                ws.cell(row=row, column=col).fill = fill("E8EBF5")
            continue
    ...
    # L13 e L14 (input manual — amarelo)
    for row in [13, 14]:
    ...
    def make_transp(nome_aba, nome_forn):
        ...
        df = df_p[df_p["Fornecedor (Nome Fantasia)"].str.upper().str.contains(nome_forn.upper(), na=False)].copy()
        df_ab = df[df["Valor a Pagar"]>0].copy()
        df_atras  = df_ab[df_ab["Previsão de Pagamento"] < HOJE].sort_values("Previsão de Pagamento")
        df_semana = df_ab[(df_ab["Previsão de Pagamento"]>=HOJE)&(df_ab["Previsão de Pagamento"]<=dom_semana)].sort_values("Previsão de Pagamento")
        df_avenc  = df_ab[df_ab["Previsão de Pagamento"]>dom_semana].sort_values("Previsão de Pagamento")
        ...
        # merge removido
        lg=wt.cell(row=1,column=9)
        lg.font=Font(name="Arial",bold=True,size=11,color=AMARELO)
        lg.alignment=Alignment(horizontal="center",vertical="center"); lg.fill=fill(AZUL)

        ta=df_atras["Valor a Pagar"].sum(); ts=df_semana["Valor a Pagar"].sum()
        tv=df_avenc["Valor a Pagar"].sum(); tg=ta+ts+tv
        ...
        for ki,(klbl,kval,kbg,kfc) in enumerate(kpis):
            cs=ki*2+1; ce=cs+1
            for r in [3,4]: wt.merge_cells(f"{get_column_letter(cs)}{r}:{get_column_letter(ce)}{r}")
            ...
            cl=wt.cell(row=4,column=cs,value=klbl)
            cl.font=fnt(sz=9,color=kfc); cl.fill=fill(kbg); cl.alignment=aln("center"); cl.border=BDR
        ...
        obs_max=df_ab["Observação"].dropna().str.len().max() if len(df_ab)>0 else 20
        wt.column_dimensions["F"].width=max(20,min(80,float(obs_max)*0.9))

        def write_sec(start, title, df_sec, title_bg, bg1, bg2):
            if len(df_sec)==0: return start
            wt.merge_cells(f"A{start}:J{start}")
            ts2=wt.cell(row=start,column=1)
            ts2.value=f"  {title}  ({len(df_sec)} fatura{'s' if len(df_sec)>1 else ''} — R$ {df_sec['Valor a Pagar'].sum():,.2f})"
            ...
            for ri,(_,row) in enumerate(df_sec.iterrows()):
                r=start+2+ri; bg=bg1 if ri%2==0 else bg2
                prev=row["Previsão de Pagamento"]
                dias=(HOJE-prev).days if pd.notna(prev) else 0
                for ci,col in enumerate(COLS,1):
                    v=row.get(col,""); c=wt.cell(row=r,column=ci)
                    c.fill=fill(bg); c.border=BDR; c.font=fnt(sz=9)
                    if col=="Valor a Pagar":
                        c.value=float(v) if v!="" else 0; c.number_format=BRL
                        c.alignment=aln("right"); c.font=fnt(sz=9,bold=True,color=VERM_TX)
                    elif col in ["Valor da Conta","Desconto"]:
                        c.value=float(v) if v!="" else 0; c.number_format=BRL; c.alignment=aln("right")
                    elif col=="Previsão de Pagamento":
                        c.value=str(prev.date()) if pd.notna(prev) else ""; c.alignment=aln("center")
        ...
        if len(df_semana)>0: wt.row_dimensions[cur].height=8; cur+=1
    ...
    # ══════════ BASES (ocultas) ════════════════════════════════════════════════
    def write_base(nome, df, val_cols, date_cols):
        ws2=wb.create_sheet(nome)
        cols=[c for c in df.columns if c not in ["mes"]]
        for ci,col in enumerate(cols,1):
        ...
        for ri,row in df.iterrows():
            for ci,col in enumerate(cols,1):
                v=row.get(col,"")
                if col in val_cols:
                    cell=ws2.cell(row=ri+2,column=ci,value=float(v) if pd.notna(v) and str(v).replace(".","").replace("-","").replace(",","").isdigit() else 0.0)
                    cell.number_format=BRL
                elif col in date_cols:
                    cell=ws2.cell(row=ri+2,column=ci,value=str(v) if pd.notna(v) else "")
    ...
    write_base("BASE_RECEBER", df_r, val_r, date_r)
    write_base("BASE_PAGAR",   df_p, val_p, date_p)
...
async def gerar_fluxo(
    csv_pagar:   UploadFile = File(..., description="CSV de contas a pagar"),
    csv_receber: UploadFile = File(..., description="CSV de contas a receber"),
):
    content_p = await csv_pagar.read()
    content_r = await csv_receber.read()

    df_p = read_csv(content_p)
    df_r = read_csv(content_r)

    xlsx_bytes = gerar_xlsx(df_p, df_r)
